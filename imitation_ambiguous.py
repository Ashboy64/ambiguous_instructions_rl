...
class ImitationLearningAmbiguous(object):
    def __init__(self, args, ):
        ...
        if getattr(args, 'multi_env', None):
            ...
            for demos, episodes in zip(args.multi_demos, args.multi_episodes):
                ...
                logger.info('loading {} of {} demos from \"{}\"'.format(episodes, demos, demos_path))
                print('loading {} of {} demos from \"{}\"'.format(episodes, demos, demos_path))
            ...
            for demos, episodes in zip(args.multi_demos, [args.val_episodes] * len(args.multi_demos)):
                ...
                logger.info('loading {} of {} valid demos from \"{demos_path_valid}\"'.format(episodes, demos, demos_path_valid))
                print('loading {} of {} valid demos from \"{demos_path_valid}\"'.format(episodes, demos, demos_path_valid))
            ...
            logger.info(f'loading demos from \"{demos_path}\"')
            self.train_demos = utils.load_demos(demos_path)
            # logger.info('loaded demos')
            logger.info(f'loaded {len(self.train_demos)} train demos')
            if args.episodes:
            ...
            logger.info(f'loading valid demos from \"{demos_path_valid}\"')
            self.val_demos = utils.load_demos(demos_path_valid)
            logger.info(f'loaded {len(self.val_demos)} val demos')
    ...
    def validate(self, episodes, verbose=True):
        ...
        for env_name in ([self.args.env] if not getattr(self.args, 'multi_env', None)
            ...
            logs += [batch_evaluate(agent, env_name, self.val_seed, episodes, nonsense=self.args.nonsense, ambiguous=self.args.ambiguous, prob_ambiguous=self.args.prob_ambiguous)]
